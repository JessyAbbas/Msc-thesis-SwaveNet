"""
SwaveNet Optuna Hyperparameter Tuning
======================================
1 seed (2024), 20 epochs, patience=5
Supports all datasets: ETTh1/h2, ETTm1/m2, electricity, weather, traffic, exchange

Usage:
    python optuna_tuning.py --data ETTh1 --gpu 0
    python optuna_tuning.py --data exchange --gpu 1
    python optuna_tuning.py --data weather --gpu 2
"""
...
def create_args(trial, base_args):
    ...
    data_path = DATA_PATH_MAP.get(base_args.data, f'{base_args.data}.csv')
    ...
    args = argparse.Namespace(
        ...
        tau=2.0,
        alpha=0.5,
        batch_size=8 if base_args.data in ['traffic','electricity'] else 32,
        lr=5e-4,
        train_epochs=20,
        patience=5,
        loss='mae',
...
def main():
    ...
    parser.add_argument('--data', type=str, required=True,
                        choices=['ETTh1', 'ETTh2', 'ETTm1', 'ETTm2',
                                 'electricity', 'weather', 'traffic', 'exchange'])
    ...
    print(f"Data file:      {DATA_PATH_MAP.get(base_args.data, base_args.data+'.csv')}")
    ...
    best_params = {
        'data': base_args.data,
        'data_path': DATA_PATH_MAP.get(base_args.data, base_args.data+'.csv'),
        'pred_len': base_args.pred_len,
    ...
    for pl in [96, 192, 336, 720]:
        print(f"python run_long.py --data {base_args.data} "
              ...
              f"--data_path {data_path} --features M "
              f"--seq_len 96 --pred_len {pl} --batch_size {8 if base_args.data in ['traffic'] else 16 if base_args.data in ['electricity'] else 32} --lr 5e-4 "
