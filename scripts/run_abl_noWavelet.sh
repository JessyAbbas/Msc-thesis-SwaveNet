#!/bin/bash
# ABLATION: noWavelet — ETTh2 + ETTm1, pl={96,336}
...
DATA="ETTh2"
mkdir -p Output/${DATA}/abl_noWavelet
for PL in 96 336; do
    echo "abl_noWavelet | ${DATA} | pl=${PL}"
    CUDA_VISIBLE_DEVICES=${GPU} python run_long.py \
        ...
        --model abl_noWavelet --keep_levels all --num_heads 8 \
        --use_gray_pe 1 --use_log_pe 1 --gray_num_bits 10 --dwc_kernel 5 \
        --save --model_name abl_noWavelet_${DATA}_pl${PL} \
        --checkpoints test_results/${DATA}/abl_noWavelet \
        2>&1 | tee "Output/${DATA}/abl_noWavelet/pl${PL}.txt"
done
