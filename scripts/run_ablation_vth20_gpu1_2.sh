#!/bin/bash
# Ablation: vth20 — ETTh1 (GPU 1) then ETTh2 (GPU 2)
cd /media/homes/abbasj/SpikF
for DATA in ETTh1 ETTh2; do
    [ "$DATA" == "ETTh1" ] && GPU=1 || GPU=2
    mkdir -p Output/${DATA}/ablation_vth20
    mkdir -p test_results/${DATA}/ablation_vth20
    for PRED_LEN in 96 336; do
        echo "ablation_vth20 | ${DATA} | pred=${PRED_LEN} | GPU ${GPU}"
        CUDA_VISIBLE_DEVICES=${GPU} python run_long.py \
            ...
            --model ablation_vth20 --keep_levels all --num_heads 8 \
            --use_gray_pe 1 --use_log_pe 1 --gray_num_bits 10 --dwc_kernel 5 \
            --save --model_name ablation_vth20_${DATA}_pl${PRED_LEN} \
            --checkpoints test_results/${DATA}/ablation_vth20 \
            2>&1 | tee "Output/${DATA}/ablation_vth20/pl${PRED_LEN}.txt"
    done
done
echo "DONE — ablation_vth20"
