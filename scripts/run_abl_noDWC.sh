#!/bin/bash
# ABLATION: noDWC — ETTh2 + ETTm1, pl={96,336}
...
DATA="ETTh2"
mkdir -p Output/${DATA}/abl_noDWC
for PL in 96 336; do
    echo "abl_noDWC | ${DATA} | pl=${PL}"
    CUDA_VISIBLE_DEVICES=${GPU} python run_long.py \
        ...
        --save --model_name abl_noDWC_${DATA}_pl${PL} \
        --checkpoints test_results/${DATA}/abl_noDWC \
        2>&1 | tee "Output/${DATA}/abl_noDWC/pl${PL}.txt"
done
