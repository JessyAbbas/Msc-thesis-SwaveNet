#!/bin/bash
# ABLATION: vth05 — ETTh2 + ETTm1, pl={96,336}
...
DATA="ETTh2"
mkdir -p Output/${DATA}/abl_vth05
for PL in 96 336; do
    echo "abl_vth05 | ${DATA} | pl=${PL}"
    CUDA_VISIBLE_DEVICES=${GPU} python run_long.py \
        ...
        --model abl_vth05 --keep_levels all --num_heads 8 \
        --use_gray_pe 1 --use_log_pe 1 --gray_num_bits 10 --dwc_kernel 5 \
        --save --model_name abl_vth05_${DATA}_pl${PL} \
        --checkpoints test_results/${DATA}/abl_vth05 \
        2>&1 | tee "Output/${DATA}/abl_vth05/pl${PL}.txt"
done
