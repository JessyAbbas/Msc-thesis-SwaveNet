...
class XNOR_SSA_GrayLog(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, tau=2.0, common_thr=0.5,
...
class SWS_LightSpikformer(nn.Module):
    ...
    def __init__(self, patch_num, D, patch_dim, tau, alpha,
        ...
        # Spiking neurons (ablation: v_threshold=0.5)
        self.generator_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend="torch", v_threshold=0.05)
        self.mp_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend="torch", v_threshold=0.5)
        self.sws_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend="torch", v_threshold=0.5)
        self.intra_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend="torch", v_threshold=0.5)
