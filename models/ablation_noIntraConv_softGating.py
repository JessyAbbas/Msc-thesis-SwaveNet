...
class SWS_LightSpikformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Wavelet Branch — SOFT GATING (ablation: replaces SpikF binary selector)
        x_t = x.transpose(-1, -2).contiguous()
        wav = self.haar_fwd(x_t)

        # Soft gating: Linear -> BN -> sigmoid (continuous [0,1] gate instead of binary spikes)
        selector = self.time2wave(x_t)
        selector = selector.flatten(0, 1)
        selector = self.bn1(selector)
        selector = selector.view(T, B, pd, D, pn)
        selector = torch.sigmoid(selector)  # Soft gate [0,1] instead of LIF spikes
        selector = selector.mean(dim=0, keepdim=True)  # Average over T instead of sum+spike
        selector = selector.repeat(T, 1, 1, 1, 1)
        ...
        # ---- Intra-conv DISABLED (ablation: noIntraConv) ----
        # y = x.flatten(0, 1)
        # y = self.intra_conv(y)
        # y = self.bn3(y)
