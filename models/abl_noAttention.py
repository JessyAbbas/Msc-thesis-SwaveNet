...
class SWS_WaveletGuidedAttention_SNN(nn.Module):
    ...
    def __init__(self, patch_num, D, patch_dim, tau, alpha,
        ...
        # ── ABLATION: noAttention — replace attention with intra+inter conv ──
        # Intra-conv (within-patch, same as original SpikF)
        self.intra_conv = nn.Conv2d(patch_dim, patch_dim, kernel_size=(5, 1), padding=(2, 0))
        self.intra_bn = nn.BatchNorm2d(patch_dim)
        self.intra_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend="torch")
        # Inter-conv (across-patch, same as original SpikF)
        self.inter_conv = nn.Conv2d(patch_dim, patch_dim, kernel_size=(1, 3), padding=(0, 1))
        self.inter_bn = nn.BatchNorm2d(patch_dim)
    ...
    def forward(self, x):
        ...
        wav_spike = self._wavelet_selector_branch(x, T, B, pd, pn, D)

        # Branch 2: wavelet_guided global mod — coarse spectral summary
        mod = self._spiking_wavelet_modulation(x, T, B, pd, pn, D)
        
        x_wav = wav_spike + x      
        x_enriched = self.enrich_lif(x_wav + mod.float())

        # ---- ABLATION: noAttention — use intra+inter conv instead ----
        # Intra-conv (within-patch mixing)
        y = x_enriched.flatten(0, 1)  # [T*B, pd, pn, D]
        y = self.intra_conv(y)
        y = self.intra_bn(y)
        y = y.view(T, B, pd, pn, D)
        y = self.intra_lif(y) + x_enriched

        # Inter-conv (across-patch mixing)
        z = y.flatten(0, 1)  # [T*B, pd, pn, D]
        z = self.inter_conv(z)
        z = self.inter_bn(z)
        z = z.view(T, B, pd, pn, D)
        z = self.inter_lif(z) + y
