# SwaveNetV1_LightSpikformer_noDWC   --   ABLATION: DWC removed
#
# Identical to SwaveNetV1_LightSpikformer except that the SpikingDWC mixer is
# gone: the class is not defined, not constructed, and not called. The
# LightSpikformerBlock is XNOR-SSA only.
#
# Haar wavelet transform with ternary spiking
# SpikF-style frequency selector
# Intra-conv: NOT PRESENT in this variant (it was dropped when the block became
#             SWS_WaveletGuidedAttention_SNN). With DWC also removed, this model
#             contains NO convolution layers at all - QKV are nn.Linear.
# Inter-patch: Light Spikformer block = XNOR-based SSA with Gray-PE + Log-PE
#              (from SeqSNN repo)
#
# NOTE: removing DWC also removes the LIF + BatchNorm1d that sat on the residual
# stream, so this variant has no normalisation between stacked blocks and its
# block output stays integer-valued. Compare at matched `blocks` when reporting.
...
def parse_keep_levels(keep_levels_str: str) -> KeepLevels:
    """"all"/"detail"/"approx"/"finest" pass through; anything else is read as
    comma-separated level indices."""
...
def generate_gray_code_matrix(M, num_bits=10):
    ...
    gray_bits = ((gray_codes.unsqueeze(-1) >> torch.arange(num_bits - 1, -1, -1, device=M.device)) & 1).float()
...
# LIGHT SPIKFORMER BLOCK (XNOR-SSA only - no DWC)

class LightSpikformerBlock(nn.Module):
    """
    XNOR-SSA (with Gray-PE + Log-PE) only. The MaxFormer-style DWC mixer that
    normally follows the attention is removed in this ablation.
    """
...
class SWS_WaveletGuidedAttention_SNN(nn.Module):
    """
    Modes:
    - "haar_residual"   : attention + high-freq wavelet residual [NEW BEST]
    - "wavelet_guided"  : original global concat (kept as baseline)
    - "patch_wavelet"   : per-patch mod
    - "sws_guided"      : wavelet selector + guided attention
    - "sws"             : original wavelet selector
    - "none"            : no wavelet
    """

    def __init__(self, patch_num, D, patch_dim, tau, alpha,
                 num_heads=8, keep_levels="all",
                 transform_spike=False, transform_vth=1.0,
                 use_gray_pe=True, use_log_pe=True,
                 gray_num_bits=10, dwc_kernel_size=5,         ## dwc_kernel_size is unused in this ablation, it will be an ignored parameter
                 ):
        super().__init__()
        ...
        # Shared Haar forward (used by all wavelet modes)
        self.haar_fwd = HaarForward(n=patch_num, spike=transform_spike,
                                     vth=transform_vth, detach_reset=True)



        if not hasattr(self, 'haar_inv'):
            self.haar_inv = HaarInverse(n=patch_num, spike=transform_spike,
                                            vth=transform_vth, detach_reset=True)
        level_mask_1d = make_haar_level_mask(patch_num, keep_levels)
        self.register_buffer("level_mask",
                                level_mask_1d.view(1, 1, 1, 1, patch_num),
                                persistent=False)
        self.time2wave = nn.Linear(patch_num, patch_num)
        self.sel_bn = nn.BatchNorm2d(patch_dim)
        self.generator_lif = MultiStepLIFNode(
            tau=tau, detach_reset=True, backend="torch", v_threshold=0.1
        )
        self.mp_lif = MultiStepLIFNode(
            tau=tau, detach_reset=True, backend="torch"
        )
        self.inv_bn = nn.BatchNorm2d(patch_dim)
        self.sws_lif = MultiStepLIFNode(
            tau=tau, detach_reset=True, backend="torch"
        )
        self.wav_proj = nn.Linear(patch_num, 1, bias=False)
        self.wav_proj_bn = nn.BatchNorm2d(patch_dim)
        self.wav_proj_lif = MultiStepLIFNode(
            tau=tau, detach_reset=True, backend="torch", v_threshold=1.0
        )
        self.enrich_lif = MultiStepLIFNode(
            tau=tau, detach_reset=True, backend="torch", v_threshold=1.0
        )

        # ── attention block (always present, always untouched) ────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    def _spiking_wavelet_modulation(self, x, T, B, pd, pn, D):
        """Original global modulation for wavelet_guided / sws_guided."""
        x_t = x.transpose(-1, -2).contiguous()
        wav_spikes = self.haar_fwd(x_t)
        mod = self.wav_proj(wav_spikes)
        mod = mod.squeeze(-1)
        mod = mod.permute(0, 1, 3, 2).contiguous()
        mod = mod.reshape(T * B * D, pd, 1, 1)
        mod = self.wav_proj_bn(mod)
        mod = mod.reshape(T, B, D, pd)
        mod = mod.permute(0, 1, 3, 2).contiguous()
        mod = mod.unsqueeze(3)
        mod = self.wav_proj_lif(mod)
        return mod

    # ─────────────────────────────────────────────────────────────────────────
    def _wavelet_selector_branch(self, x, T, B, pd, pn, D):
        """Original SpikF-style wavelet selector (unchanged)."""
        x_t = x.transpose(-1, -2).contiguous()
        wav = self.haar_fwd(x_t)
        selector = self.time2wave(x_t)
        selector = selector.flatten(0, 1)
        selector = self.sel_bn(selector)
    ...
    def forward(self, x):
        ...
        # Branch 1: sws selector — data-dependent frequency band selection

        wav_spike = self._wavelet_selector_branch(x, T, B, pd, pn, D) # [T, B, pd, pn, D]  {0,1}

        # Branch 2: wavelet_guided global mod — coarse spectral summary

        mod = self._spiking_wavelet_modulation(x, T, B, pd, pn, D) # mod: [T, B, pd, 1, D]  {0,1}
        
        x_wav = wav_spike + x      
        #x_enriched = x_wav + mod.float()                   # {0,1,2,3}
        x_enriched = self.enrich_lif(x_wav + mod.float())


        # Attention on enriched input
        z = x_enriched.permute(0, 1, 4, 3, 2).contiguous()
        T_, B_, D_feat, pn_, pd_ = z.shape
        z = z.reshape(T_, B_ * D_feat, pn_, pd_)
        z = self.light_spikformer(z)
        z = z.reshape(T_, B_, D_feat, pn_, pd_)
        z = z.permute(0, 1, 4, 3, 2).contiguous()
        z = z + res_x
...
class SwaveNetV1_LightSpikformer(nn.Module):
    def __init__(self, input_len, patch_num, patch_dim, T, blocks, D, pred_len,
        ...
        self.SWSs = nn.ModuleList([
            SWS_WaveletGuidedAttention_SNN(
                patch_num=patch_num, D=D, patch_dim=patch_dim, tau=tau,
                alpha=alpha, num_heads=num_heads, keep_levels=keep_levels,
                transform_spike=transform_spike, transform_vth=2.0,
                use_gray_pe=use_gray_pe, use_log_pe=use_log_pe,
                gray_num_bits=gray_num_bits,
            )
