...
class HaarForward(nn.Module):
    def __init__(self, n, spike=True, vth=1.0, detach_reset=True):
        super().__init__()
        self.n = n
...
class XNOR_SSA_GrayLog(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, tau=2.0, common_thr=1.0,
        ...
        self.q_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
        
        self.k_m = nn.Linear(dim, dim)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
        
        self.v_m = nn.Linear(dim, dim)
        self.v_bn = nn.BatchNorm1d(dim)
        self.v_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
        
        # Attention LIF (v_threshold = common_thr/2, from actual code)
        self.attn_lif = MultiStepLIFNode(tau=tau, v_threshold=common_thr / 2,
                                          detach_reset=False, backend="torch")
        
        # Output projection
        self.last_m = nn.Linear(dim, dim)
        self.last_bn = nn.BatchNorm1d(dim)
        self.last_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
...
class SpikingDWC(nn.Module):
    ...
    def __init__(self, dim, tau=2.0, kernel_size=5):
        ...
        self.conv_neuron = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
...
class SWS_LightSpikformer(nn.Module):
    ...
    def __init__(self, patch_num, D, patch_dim, tau, alpha,
        ...
        # Spiking neurons
        self.generator_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch", v_threshold=0.1)
        self.mp_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
        self.sws_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
        self.intra_lif = MultiStepLIFNode(tau=tau, detach_reset=False, backend="torch")
...
class SPE(nn.Module):
    def __init__(self, input_len, patch_num, patch_dim, T, tau, D):
        ...
        self.encoder_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='torch')
        self.D = D
        self.T = T
        self.patch_dim = patch_dim
        self.patch_num = patch_num
